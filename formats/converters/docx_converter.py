...
# Именованный логгер без basicConfig: root-логгер не трогаем,
# иначе DEBUG-шум сторонних библиотек заливает консоль при пакетной обработке
...
class DocxConverter(BaseConverter):
    ...
    def _get_list_info(self, para: Paragraph):
        ...
        # Получаем определения нумерации из документа
        try:
            ...
            # Ищем уровень (lvl) с нужным ilvl
            lvl = None
            ...
            logger.debug(f"Ошибка при получении определений нумерации: {e}")
            return True, True, level

    def _get_list_indent(self, para: Paragraph) -> int:
        """Определяет уровень вложенности по отступу (если нет numPr)."""
        try:
            num_pr = para._element.find(qn("w:numPr"))
            if num_pr is None and para.style:
                style_elem = para.style.element
    ...
    def _apply_style(self, text: str, style_name: str, para: Paragraph) -> str:
        ...
        # Получаем информацию о списке (XML)
        ...
        # Если распознан как список через XML.
        # Поиск формата нумерации в numbering.xml может не сработать и вернуть
        # fallback is_numbered=True — поэтому имя стиля с "bullet"/"маркированный"
        # имеет приоритет: такой список точно маркированный.
        if is_list:
            style_says_bullet = any(
                key in style_lower for key in ("bullet", "маркированный")
            )
            if is_numbered and not style_says_bullet:
                return f"{indent}1. {text}"
            else:
                return f"{indent}- {text}"

        # Эвристика по тексту (цифра в начале)
        ...
        # Эвристика по имени стиля (для документов без numPr)
