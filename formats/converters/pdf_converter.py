...
from formats.config import IMAGE_PLACEHOLDER, OCR_FAILED_PLACEHOLDER, PDF_MIN_CHARS_FOR_TEXT, PDF_OCR_DPI
from formats.converters.base import BaseConverter, ConversionResult
from formats.utils.ocr import ocr_pdf_page_image, is_tesseract_available
...
class PdfConverter(BaseConverter):
    ...
    def convert(self, file_path: Path) -> ConversionResult:
        ...
        # pdfplumber открываем ОДИН раз на документ (нужен для таблиц и буллетов).
        # Раньше он открывался заново на каждую страницу дважды — на больших PDF
        # это давало квадратичное время работы.
        try:
            pl_pdf = pdfplumber.open(str(file_path))
        except Exception as e:
            pl_pdf = None
            result.add_warning(f"pdfplumber не смог открыть файл: {e}")

        try:
            for page_num, page in enumerate(doc, start=1):
                pl_page = pl_pdf.pages[page_num - 1] if pl_pdf else None
                page_text = self._convert_page(page, page_num, base_font_size, result, pl_page)
                if page_text.strip():
    ...
    def _convert_page(
        ...
        result: ConversionResult, pl_page=None,
    ) -> str:
        """
        ...
        return self._extract_formatted_text(page, page_num, base_font_size, result, pl_page)
    ...
    def _extract_formatted_text(
        ...
        table_bboxes, table_md = self._process_page_tables(page, result, pl_page)
        ...
        all_lines = self._inject_bullets_from_pdfplumber(all_lines, pl_page)
    ...
    def _process_spans(self, spans: list[dict], base_font_size: float) -> str:
        """
        Обрабатывает spans строки.

        КЛЮЧЕВОЕ:
        - НЕ применяет markdown к каждому span отдельно
        - сначала объединяет spans
        - затем применяет markdown к цельным блокам
        """
        ...
        line_text = self._format_spans_to_md(processed_spans).strip()
    ...
    def _format_spans_to_md(self, spans_meta: list[dict]) -> str:
        """
        Объединяет соседние spans с одинаковым форматированием
        → устраняет **имеет ** **мало**
        """
    ...
    def _process_page_tables(
        self, page: fitz.Page, result: ConversionResult, pl_page=None
    ) -> tuple[list[fitz.Rect], list[str]]:
        """
        По странице pdfplumber (открытой один раз в convert()) находит реальные таблицы и:
